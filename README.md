...
- You should clamp the border (edge) pixels with white color as in GL_CLAMP_TO_BORDER (you can use any color as long the your implementation remains consistent), but we will give full credit for ones who correctly implement GL_CLAMP_TO_EDGE, clamping texture values to the nearest valid image pixel (NOTE: Clamping to border is easier than clamping to edge!).
